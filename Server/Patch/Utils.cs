...
namespace Patch
{
    public abstract class Utils
    {
        ...
        {
            if (string.Compare(ipString, "any", true) == 0)
            {
                return IPAddress.Any;
            }
            else if (string.Compare(ipString, "localhost", true) == 0)
            {
                return IPAddress.Loopback;
            }
            else if (string.Compare(ipString, "auto", true) == 0)
            {
                try
                {
                    IPHostEntry iphe;
            ...
            else if (Regex.IsMatch(ipString, "^([0-9]{1,3}\\.){3}[0-9]{1,3}$"))
            {
                try
                {
        ...
        public static void StringRemoveNull(ref string str)
        {
            int pos = str.IndexOf('\0', 0);
            if (pos > 0)
            {
        ...
        public static uint CRC32(byte[] sourceArray, int sourceIndex, int length)
        {
            uint crc = 0xFFFFFFFF;
            for (int i1 = sourceIndex; i1 < (sourceIndex + length); i1++)
            {
                crc = (crc >> 8) ^ CRC32_Table[(crc ^ sourceArray[i1]) & 0xFF];
            }
            crc = (crc ^ 0xFFFFFFFF);
            return crc;
        }
